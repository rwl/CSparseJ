...
public class Zcs_dfs {
     ...
     */
    public static int cs_dfs(int j, Zcs G, int top, int[] xi, int xi_offset, int[] pstack, int pstack_offset,
            int[] pinv, int pinv_offset) {
        int i, p, p2, jnew, head = 0, Gp[], Gi[];
        boolean done;
        if (!Zcs_util.CS_CSC(G) || xi == null || pstack == null)
        ...
        while (head >= 0) {
            ...
            if (!Zcs_util.CS_MARKED(Gp, j)) {
                Zcs_util.CS_MARK(Gp, j); /* mark node j as visited */
                pstack[pstack_offset + head] = (jnew < 0) ? 0 : Zcs_util.CS_UNFLIP(Gp[jnew]);
            }
            done = true; /* node j done if no unvisited neighbors */
            p2 = (jnew < 0) ? 0 : Zcs_util.CS_UNFLIP(Gp[jnew + 1]);
            for (p = pstack[pstack_offset + head]; p < p2; p++) /* examine all neighbors of j */
            {
                i = Gi[p]; /* consider neighbor node i */
                if (Zcs_util.CS_MARKED(Gp, i))
