...
public class Zcs_util {
     ...
     */
    public static Zcs cs_spalloc(int m, int n, int nzmax, boolean values, boolean triplet) {
        Zcs A = new Zcs(); /* allocate the Zcs struct */
     ...
     */
    public static boolean cs_sprealloc(Zcs A, int nzmax) {
        if (A == null)
            return (false);
        if (nzmax <= 0)
            nzmax = (Zcs_util.CS_CSC(A)) ? (A.p[A.n]) : A.nz;
        int[] Ainew = new int[nzmax];
        int length = Math.min(nzmax, A.i.length);
        System.arraycopy(A.i, 0, Ainew, 0, length);
        A.i = Ainew;
        if (Zcs_util.CS_TRIPLET(A)) {
     ...
     */
    public static Zcsd cs_dalloc(int m, int n) {
        Zcsd D;
        D = new Zcsd();
        D.p = new int[m];
        D.r = new int[m + 6];
     ...
     */
    public static boolean CS_CSC(Zcs A) {
     ...
     */
    public static boolean CS_TRIPLET(Zcs A) {
