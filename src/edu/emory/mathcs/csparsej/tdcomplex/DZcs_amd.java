...
public class DZcs_amd {
     ...
     */
    public static int[] cs_amd(int order, DZcs A) {
        DZcs C, A2, AT;
        ...
        if (!DZcs_util.CS_CSC(A) || order <= 0 || order > 3)
            return (null); /* check */
        AT = DZcs_transpose.cs_transpose(A, false); /* compute A' */
        if (AT == null)
            return (null);
        m = A.m;
        n = A.n;
        ...
        if (order == 1 && n == m) {
            C = DZcs_add.cs_add(A, AT, DZcs_complex.cs_czero(), DZcs_complex.cs_czero()); /* C = A+A' */
        } else if (order == 2) {
            ...
            A2 = DZcs_transpose.cs_transpose(AT, false); /* A2 = AT' */
            C = (A2 != null) ? DZcs_multiply.cs_multiply(AT, A2) : null; /* C=A'*A with no dense rows */
            A2 = null;
        } else {
            C = DZcs_multiply.cs_multiply(AT, A); /* C=A'*A */
        }
        AT = null;
        if (C == null)
            return (null);
        DZcs_fkeep.cs_fkeep(C, new Cs_diag(), null); /* drop diagonal entries */
        Cp = C.p;
        cnz = Cp[n];
        P = new int[n + 1]; /* allocate result */
        W = new int[8 * (n + 1)]; /* get workspace */
        t = cnz + cnz / 5 + 2 * n; /* add elbow room to C */
        DZcs_util.cs_sprealloc(C, t);
        len = W;
        nv = W;
        int nv_offset = n + 1;
        next = W;
        ...
        while (nel < n) /* while (selecting pivots) do */
        {
            ...
            if (elenk > 0 && cnz + mindeg >= nzmax) {
                for (j = 0; j < n; j++) {
                    if ((p = Cp[j]) >= 0) /* j is a live node or element */
                    {
                        ...
                        Ci[p] = DZcs_util.CS_FLIP(j); /* first entry is now CS_FLIP(j) */
                    }
                }
                for (q = 0, p = 0; p < cnz;) /* scan all of memory */
                {
                    if ((j = DZcs_util.CS_FLIP(Ci[p++])) >= 0) /* found object j */
                    {
            ...
            for (k1 = 1; k1 <= elenk + 1; k1++) {
                ...
                if (e != k) {
                    Cp[e] = DZcs_util.CS_FLIP(k); /* absorb e into k */
            ...
            for (pk = pk1; pk < pk2; pk++) /* scan2: degree update */
            {
                ...
                for (h = 0, d = 0, p = p1; p <= p2; p++) /* scan Ei */
                {
                    ...
                    if (w[w_offset + e] != 0) /* e is an unabsorbed element */
                    {
                        ...
                        if (dext > 0) {
                            ...
                            Cp[e] = DZcs_util.CS_FLIP(k); /* aggressive absorb. e.k */
                ...
                {
                    Cp[i] = DZcs_util.CS_FLIP(k); /* absorb i into k */
                    nvi = -nv[nv_offset + i];
                    dk -= nvi; /* |Lk| -= |i| */
            ...
            for (pk = pk1; pk < pk2; pk++) {
                ...
                for (; i != -1 && next[next_offset + i] != -1; i = next[next_offset + i], mark++) {
                    ...
                    for (j = next[next_offset + i]; j != -1;) /* compare i with all j */
                    {
                        ...
                        {
                            Cp[j] = DZcs_util.CS_FLIP(i); /* absorb j into i */
        ...
        /* --- Postordering ----------------------------------------------------- */
        for (i = 0; i < n; i++)
            Cp[i] = DZcs_util.CS_FLIP(Cp[i]);/* fix assembly tree */
        for (j = 0; j <= n; j++)
