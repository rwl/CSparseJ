...
public class Zcs_add {
     ...
     */
    public static Zcs cs_add(Zcs A, Zcs B, Complex alpha, Complex beta) {
        int p, j, nz = 0, anz;
        ...
        for (j = 0; j < n; j++) {
            ...
            nz = Zcs_scatter.cs_scatter(A, j, alpha, w, x, j + 1, C, nz); /* alpha*A(:,j)*/
            nz = Zcs_scatter.cs_scatter(B, j, beta, w, x, j + 1, C, nz); /* beta*B(:,j) */
            if (values)
                for (p = Cp[j]; p < nz; p++)
                    Cx[p] = x[Ci[p]];
        }
