...
public class Zcs_reach {
     ...
     */
    public static int cs_reach(Zcs G, Zcs B, int k, int[] xi, int[] pinv) {
        int p, n, top, Bp[], Bi[], Gp[];
        if (!Zcs_util.CS_CSC(G) || !Zcs_util.CS_CSC(B) || xi == null)
        ...
        for (p = Bp[k]; p < Bp[k + 1]; p++) {
            if (!Zcs_util.CS_MARKED(Gp, Bi[p])) /* start a dfs at unmarked node i */
            {
                top = Zcs_dfs.cs_dfs(Bi[p], G, top, xi, 0, xi, n, pinv, 0);
            }
        }
        for (p = top; p < n; p++)
            Zcs_util.CS_MARK(Gp, xi[p]); /* restore G */
        return (top);
    }
