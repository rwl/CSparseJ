...
public class Zcs_spsolve {
     ...
     */
    public static int cs_spsolve(Zcs G, Zcs B, int k, int[] xi, Complex[] x, int[] pinv, boolean lo) {
        int j, J, p, q, px, top, n, Gp[], Gi[], Bp[], Bi[];
        Complex Gx[], Bx[];
        if (!Zcs_util.CS_CSC(G) || !Zcs_util.CS_CSC(B) || xi == null || x == null)
            return (-1);
        Gp = G.p;
        Gi = G.i;
        Gx = G.x;
        ...
        Bi = B.i;
        Bx = B.x;
        top = Zcs_reach.cs_reach(G, B, k, xi, pinv); /* xi[top..n-1]=Reach(B(:,k)) */
        for (p = top; p < n; p++)
