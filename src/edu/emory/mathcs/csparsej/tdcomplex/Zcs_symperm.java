...
public class Zcs_symperm {
     ...
     */
    public static Zcs cs_symperm(Zcs A, int[] pinv, boolean values) {
        ...
        Ai = A.i;
        Ax = A.x;
        C = Zcs_util.cs_spalloc(n, n, Ap[n], values && (Ax != null), false); /* alloc result*/
        ...
        Zcs_cumsum.cs_cumsum(Cp, w, n); /* compute column pointers of C */
        for (j = 0; j < n; j++) {
            j2 = pinv != null ? pinv[j] : j; /* column j of A is column j2 of C */
            for (p = Ap[j]; p < Ap[j + 1]; p++) {
                i = Ai[p];
