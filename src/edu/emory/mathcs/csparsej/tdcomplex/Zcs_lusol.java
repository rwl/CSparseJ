...
public class Zcs_lusol {
     ...
     */
    public static boolean cs_lusol(int order, Zcs A, Complex[] b, double tol) {
        Complex[] x;
        Zcss S;
        Zcsn N;
        int n;
        ...
        n = A.n;
        S = Zcs_sqr.cs_sqr(order, A, false); /* ordering and symbolic analysis */
        N = Zcs_lu.cs_lu(A, S, tol); /* numeric LU factorization */
        x = new Complex[n]; /* get workspace */
        ok = (S != null && N != null);
        if (ok) {
            Zcs_ipvec.cs_ipvec(N.pinv, b, x, n); /* x = b(p) */
            Zcs_lsolve.cs_lsolve(N.L, x); /* x = L\x */
            Zcs_usolve.cs_usolve(N.U, x); /* x = U\x */
            Zcs_ipvec.cs_ipvec(S.q, x, b, n); /* b(q) = x */
        }
        return (ok);
    }
