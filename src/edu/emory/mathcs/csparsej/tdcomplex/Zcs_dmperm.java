 ...
 *
 */
public class Zcs_dmperm {
    /* breadth-first search for coarse decomposition (C0,C1,R1 or R0,R3,C3) */
    private static boolean cs_bfs(Zcs A, int n, int[] wi, int[] wj, int[] queue, int[] imatch, int imatch_offset,
        ...
        C = (mark == 1) ? A : Zcs_transpose.cs_transpose(A, false);
        if (C == null)
    ...
    private static class Cs_rprune implements Zcs_ifkeep {
     ...
     */
    public static Zcsd cs_dmperm(Zcs A, int seed) {
        ...
        m = A.m;
        n = A.n;
        D = Zcs_util.cs_dalloc(m, n); /* allocate result */
        if (D == null)
            return (null);
        p = D.p;
        q = D.q;
        ...
        cc = D.cc;
        rr = D.rr;
        jmatch = Zcs_maxtrans.cs_maxtrans(A, seed); /* max transversal */
        ...
        jmatch = null;
        /* --- Fine decomposition ----------------------------------------------- */
        pinv = Zcs_pinv.cs_pinv(p, m); /* pinv=p' */
        if (pinv == null)
            return (null);
        C = Zcs_permute.cs_permute(A, pinv, q, false);/* C=A(p,q) (it will hold A(R2,C2)) */
        pinv = null;
        if (C == null)
            return (null);
        Cp = C.p;
        ...
        {
            Zcs_fkeep.cs_fkeep(C, new Cs_rprune(), rr);
            cnz = Cp[nc];
            Ci = C.i;
            if (rr[1] > 0)
                for (k = 0; k < cnz; k++)
        ...
        C.m = nc;
        scc = Zcs_scc.cs_scc(C); /* find strongly connected components of C*/
        if (scc == null)
            return (null);
