...
public class Zcs_transpose {
     ...
     */
    public static Zcs cs_transpose(Zcs A, boolean values) {
        ...
        Ai = A.i;
        Ax = A.x;
        C = Zcs_util.cs_spalloc(n, m, Ap[n], values && (Ax != null), false); /* allocate result */
        ...
        for (p = 0; p < Ap[n]; p++)
            w[Ai[p]]++; /* row counts */
        Zcs_cumsum.cs_cumsum(Cp, w, m); /* row pointers */
        for (j = 0; j < n; j++) {
