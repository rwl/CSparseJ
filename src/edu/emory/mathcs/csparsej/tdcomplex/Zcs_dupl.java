...
public class Zcs_dupl {
     ...
     */
    public static boolean cs_dupl(Zcs A) {
        ...
        for (j = 0; j < n; j++) {
            ...
            for (p = Ap[j]; p < Ap[j + 1]; p++) {
                ...
                if (w[i] >= q) {
                    Ax[w[i]] = Ax[w[i]].add(Ax[p]); /* A(i,j) is a duplicate */
                } else {
        ...
        Ap[n] = nz; /* finalize A */
        return Zcs_util.cs_sprealloc(A, 0); /* remove extra space from A */
    }
