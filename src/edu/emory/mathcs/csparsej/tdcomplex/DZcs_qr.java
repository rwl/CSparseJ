...
public class DZcs_qr {
    ...
    public static DZcsn cs_qr(DZcs A, DZcss S) {
        ...
        x = new Complex[m2]; /* get double workspace */
        ...
        for (k = 0; k < n; k++) /* compute V and R */
        {
            ...
            for (p = top; p < n; p++) /* for each i in pattern of R(:,k) */
            {
                ...
                x[i] = Complex.ZERO;
                if (parent[i] == k)
                    vnz = DZcs_scatter.cs_scatter(V, i, Complex.ZERO, w, null, k, V, vnz);
            }
            ...
            vx[0] = Vx[p1];
            Rx[rnz++] = DZcs_house.cs_house(vx, beta, vnz - p1); /* [v,beta]=house(x) */
            Beta[k] = beta[0];
        }
        Rp[n] = rnz; /* finalize R */
        Vp[n] = vnz; /* finalize V */
