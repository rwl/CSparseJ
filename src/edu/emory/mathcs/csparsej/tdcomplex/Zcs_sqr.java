 ...
 * 
 */
public class Zcs_sqr {
    /* compute nnz(V) = S->lnz, S->pinv, S->leftmost, S->m2 from A and S->parent */
    private static boolean cs_vcount(Zcs A, Zcss S) {
     ...
     */
    public static Zcss cs_sqr(int order, Zcs A, boolean qr) {
        int n, k, post[];
        Zcss S;
        boolean ok = true;
        ...
        S.q = Zcs_amd.cs_amd(order, A); /* fill-reducing ordering */
        if (order > 0 && S.q == null)
            return (null);
        if (qr) /* QR symbolic analysis */
        {
            Zcs C = order > 0 ? Zcs_permute.cs_permute(A, null, S.q, false) : A;
            S.parent = Zcs_etree.cs_etree(C, true); /* etree of C'*C, where C=A(:,q) */
            post = Zcs_post.cs_post(S.parent, n);
            S.cp = Zcs_counts.cs_counts(C, S.parent, post, true); /* col counts chol(C'*C) */
