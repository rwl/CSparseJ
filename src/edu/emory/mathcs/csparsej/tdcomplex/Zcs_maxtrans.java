 ...
 * 
 */
public class Zcs_maxtrans {
    /* find an augmenting path starting at column k and extend the match if found */
    private static void cs_augment(int k, Zcs A, int[] jmatch, int jmatch_offset, int[] cheap, int cheap_offset,
     ...
     */
    public static int[] cs_maxtrans(Zcs A, int seed) /*[jmatch [0..m-1]; imatch [0..n-1]]*/
    {
        ...
        for (i = 0; i < m; i++)
            m2 += w[i];
        C = (m2 < n2) ? Zcs_transpose.cs_transpose(A, false) : A; /* transpose if needed */
        if (C == null)
            return null;
        n = C.n;
        m = C.m;
        ...
        q = Zcs_randperm.cs_randperm(n, seed); /* q = random permutation */
