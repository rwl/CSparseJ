...
public class Zcs_counts {
    ...
    private static int[] init_ata(Zcs AT, int[] post, int[] w) {
     ...
     */
    public static int[] cs_counts(Zcs A, int[] parent, int[] post, boolean ata) {
        ...
        Zcs AT;
        if (!Zcs_util.CS_CSC(A) || parent == null || post == null)
        ...
        for (k = 0; k < n; k++) {
            ...
            for (J = HEAD(k, j, head, head_offset, ata); J != -1; J = NEXT(J, next, next_offset, ata)) /* J=j for LL'=A case */
            {
                for (p = ATp[J]; p < ATp[J + 1]; p++) {
                    i = ATi[p];
                    q = Zcs_leaf.cs_leaf(i, j, first, first_offset, maxfirst, maxfirst_offset, prevleaf,
