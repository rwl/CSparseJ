...
public class DZcs_dmperm {
    ...
    private static boolean cs_bfs(DZcs A, int n, int[] wi, int[] wj, int[] queue, int[] imatch, int imatch_offset,
        ...
        C = (mark == 1) ? A : DZcs_transpose.cs_transpose(A, false);
        if (C == null)
    ...
    public static DZcsd cs_dmperm(DZcs A, int seed) {
        ...
        m = A.m;
        n = A.n;
        D = DZcs_util.cs_dalloc(m, n); /* allocate result */
        if (D == null)
            return (null);
        p = D.p;
        q = D.q;
        ...
        cc = D.cc;
        rr = D.rr;
        jmatch = DZcs_maxtrans.cs_maxtrans(A, seed); /* max transversal */
        ...
        jmatch = null;
        /* --- Fine decomposition ----------------------------------------------- */
        pinv = DZcs_pinv.cs_pinv(p, m); /* pinv=p' */
        if (pinv == null)
            return (null);
        C = DZcs_permute.cs_permute(A, pinv, q, false);/* C=A(p,q) (it will hold A(R2,C2)) */
        pinv = null;
        if (C == null)
            return (null);
        Cp = C.p;
        ...
        {
            DZcs_fkeep.cs_fkeep(C, new Cs_rprune(), rr);
            cnz = Cp[nc];
            Ci = C.i;
            if (rr[1] > 0)
                for (k = 0; k < cnz; k++)
        ...
        C.m = nc;
        scc = DZcs_scc.cs_scc(C); /* find strongly connected components of C*/
        if (scc == null)
            return (null);
