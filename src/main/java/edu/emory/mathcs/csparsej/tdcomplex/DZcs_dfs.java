...
public class DZcs_dfs {
    ...
    public static int cs_dfs(int j, DZcs G, int top, int[] xi, int xi_offset, int[] pstack, int pstack_offset,
        ...
        boolean done;
        if (!DZcs_util.CS_CSC(G) || xi == null || pstack == null)
        ...
        while (head >= 0) {
            ...
            if (!DZcs_util.CS_MARKED(Gp, j)) {
                DZcs_util.CS_MARK(Gp, j); /* mark node j as visited */
                pstack[pstack_offset + head] = (jnew < 0) ? 0 : DZcs_util.CS_UNFLIP(Gp[jnew]);
            }
            done = true; /* node j done if no unvisited neighbors */
            p2 = (jnew < 0) ? 0 : DZcs_util.CS_UNFLIP(Gp[jnew + 1]);
            for (p = pstack[pstack_offset + head]; p < p2; p++) /* examine all neighbors of j */
            {
                i = Gi[p]; /* consider neighbor node i */
                if (DZcs_util.CS_MARKED(Gp, i))
