...
import static edu.emory.mathcs.csparsej.tdcomplex.DZcs_util.CS_FLIP ;
...
public class DZcs_amd {
	...
	public static int[] cs_amd(int order, DZcs A)
	{
		...
		int Cp[], Ci[], last[], W[], len[], nv[], next[], P[], head[], elen[], degree[], w[],
			...
			cnz, nel = 0, p, p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, n, m, t ;
		/* FIXME: unsigned */int h ;
		boolean ok ;
		/* --- Construct matrix C ----------------------------------------------- */
		...
		while (nel < n)					/* while (selecting pivots) do */
		{
			...
			if (next [next_offset + k] != -1) last [next [next_offset + k]] = -1 ;
			...
			for (k1 = 1 ; k1 <= elenk + 1 ; k1++)
			{
				...
				{
					i = Ci [pj++] ;
					if ((nvi = nv [nv_offset + i]) <= 0) continue; /* node i dead, or seen */
			...
			{
				i = Ci [pk] ;
				if (nv [nv_offset + i] >= 0) continue;	/* skip if i is dead */
		...
		{
			if (nv [nv_offset + j] > 0) continue ;		/* skip if j is an element */
			next [next_offset + j] = head [head_offset + Cp [j]] ; /* place j in list of its parent */
			head [head_offset + Cp [j]] = j ;
		}
		for (e = n ; e >= 0 ; e--)				/* place elements in lists */
		{
			if (nv [nv_offset + e] <= 0) continue;		/* skip unless e is an element */
			if (Cp [e] != -1)
			{
