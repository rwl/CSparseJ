...
public class DZcs_scc {
    ...
    public static DZcsd cs_scc(DZcs A) {
        ...
        n = A.n;
        Ap = A.p;
        D = DZcs_util.cs_dalloc(n, 0); /* allocate result */
        AT = DZcs_transpose.cs_transpose(A, false); /* AT = A' */
        ...
        {
            if (!DZcs_util.CS_MARKED(Ap, i))
                top = DZcs_dfs.cs_dfs(i, A, top, xi, 0, pstack, pstack_offset, null, 0);
        }
        for (i = 0; i < n; i++)
            DZcs_util.CS_MARK(Ap, i); /* restore A; unmark all nodes*/
        top = n;
        nb = n;
        for (k = 0; k < n; k++) /* dfs(A') to find strongly connnected comp */
        {
            i = xi[k]; /* get i in reverse order of finish times */
            if (DZcs_util.CS_MARKED(ATp, i))
                continue; /* skip node i if already ordered */
            r[nb--] = top; /* node i is the start of a component in p */
            top = DZcs_dfs.cs_dfs(i, AT, top, p, 0, pstack, pstack_offset, null, 0);
        }
