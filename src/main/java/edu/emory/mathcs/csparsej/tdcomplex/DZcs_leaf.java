...
public class DZcs_leaf {
		    ...
		    int[] prevleaf, int prevleaf_offset, int[] ancestor, int ancestor_offset, int[] jleaf)
	{
		...
		if (first == null || maxfirst == null || prevleaf == null || ancestor == null || jleaf == null) return (-1) ;
		jleaf[0] = 0 ;
		if (i <= j || first [first_offset + j] <= maxfirst [maxfirst_offset + i]) return (-1) ;  /* j not a leaf */
		...
		if (jleaf[0] == 1) return (i) ;			/* if 1st leaf, q = root of ith subtree */
