...
public class DZcs_sqr {
    ...
    private static boolean cs_vcount(DZcs A, DZcss S) {
     ...
     */
    public static DZcss cs_sqr(int order, DZcs A, boolean qr) {
        int n, k, post[];
        DZcss S;
        boolean ok = true;
        ...
        S.q = DZcs_amd.cs_amd(order, A); /* fill-reducing ordering */
        if (order > 0 && S.q == null)
            return (null);
        if (qr) /* QR symbolic analysis */
        {
            DZcs C = order > 0 ? DZcs_permute.cs_permute(A, null, S.q, false) : A;
            S.parent = DZcs_etree.cs_etree(C, true); /* etree of C'*C, where C=A(:,q) */
            post = DZcs_post.cs_post(S.parent, n);
            S.cp = DZcs_counts.cs_counts(C, S.parent, post, true); /* col counts chol(C'*C) */
